...
public class ParquetWriteFile {
  ...
  private static Schema parseSchema() {
    ...
    try {
      // pass path to schema
      InputStream systemResourceAsStream = ClassLoader.getSystemResourceAsStream("EmpSchema.avsc");
  ...
  private static void writeToParquet(List<GenericData.Record> recordList, Schema schema) {
    ...
    try {
      writer = AvroParquetWriter.<GenericData.Record>builder(path).withRowGroupSize(ParquetWriter.DEFAULT_BLOCK_SIZE)
